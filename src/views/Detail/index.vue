          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(item,index) in spuSaleAttrList" :key="index">
                ...
                <dd changepirce="0" :class="{active:i.isChecked == 1}" v-for="(i,index) in item.spuSaleAttrValueList" :key="index" @click="changeAttr(i,item.spuSaleAttrValueList)">
                  {{i.saleAttrValueName}}
                </dd>
              </dl>
            </div>
...
<script>
  ...
  export default {
    ...
    computed:{
      ...mapState('detailOptions',['goodsInfo']),
      // 简化goodsInfo数据
      ...mapGetters('detailOptions',['categoryView','skuInfo','spuSaleAttrList'])
    },
    mounted() {
      // Detail组件挂载后 派发actions 获取数据
      this.$store.dispatch('detailOptions/getGoodsInfo',this.skuId)
    },
    methods: {
      changeAttr(value,list){
        list.forEach(item => {
            item.isChecked = '0'
        });
        value.isChecked = '1'
      ...
      async addShopCart(){
        ...
        // 成功
        try {
          await this.$store.dispatch('detailOptions/addShopCart',{skuId:this.skuId,skuNum:this.skuNum})
          // 跳转前 将要传给下一级组件的skuInfo 保存在会话存储中
